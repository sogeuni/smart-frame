# Smart Frame OneDrive 설정

이 앱은 OneDrive에 로그인한 후 사용자가 선택한 폴더에 들어 있는 이미지를
무작위 순서로 화면에 표시합니다.
...
2. Android 플랫폼을 추가한 후 패키지 이름을 `dev.sogn.smartframe`으로
   입력하고, APK 서명에 사용한 인증서의 서명 해시를 등록합니다.
3. Microsoft Graph의 위임 권한 `Files.Read`를 추가합니다.
4. `app/src/main/res/raw/auth_config_single_account.json`에서
   `REPLACE_WITH_ENTRA_CLIENT_ID`와
   `REPLACE_WITH_URL_ENCODED_SIGNATURE_HASH`를 실제 값으로 교체합니다.
5. `app/src/main/AndroidManifest.xml`에서
   `REPLACE_WITH_SIGNATURE_HASH`를 실제 서명 해시로 교체합니다. Manifest에
   입력하는 서명 해시는 URL 인코딩하면 안 됩니다.
