...
enum class DisplayMode(val preferenceValue: String) {
    WEBVIEW("webview"),
    IMAGE("image");

    companion object {
        fun fromPreference(value: String?): DisplayMode =
            entries.firstOrNull { it.preferenceValue == value } ?: WEBVIEW
    }
}

data class SmartFrameConfig(
    val displayMode: DisplayMode = DisplayMode.WEBVIEW,
...
object SmartFramePreferences {
    ...
    fun load(context: Context): SmartFrameConfig {
        ...
        return SmartFrameConfig(
            displayMode = DisplayMode.fromPreference(
                preferences.getString(KEY_DISPLAY_MODE, DisplayMode.WEBVIEW.preferenceValue),
            ),
    ...
    fun save(context: Context, config: SmartFrameConfig) {
        context.getSharedPreferences(PREFERENCES_NAME, Context.MODE_PRIVATE)
            .edit {
                putString(KEY_DISPLAY_MODE, config.displayMode.preferenceValue)
    ...
    fun normalizeUrl(value: String): String? {
        ...
        return candidate.takeIf { isAllowedWebUrl(uri) }
    }

    fun isAllowedWebUrl(uri: android.net.Uri): Boolean =
        !uri.host.isNullOrBlank() &&
            (
                uri.scheme.equals("https", true) ||
                    (
                        uri.scheme.equals("http", true) &&
                            isPrivateIpv4Address(uri.host.orEmpty())
                        )
                )

    internal fun isPrivateIpv4Address(host: String): Boolean {
    ...
    fun isReady(context: Context): Boolean {
        ...
        val contentReady = when (config.displayMode) {
            DisplayMode.WEBVIEW -> normalizeUrl(config.url) != null
            DisplayMode.IMAGE -> config.oneDriveFolderId.isNotBlank()
        }
        return contentReady &&
